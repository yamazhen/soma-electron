...
interface Props {
  setQuizInReview: (quiz: QuizData) => void;
}
...
const QuizReview: React.FC<Props> = ({ setQuizInReview }) => {
  ...
  const startGeneralReview = () => {
    // This would combine questions from all quizzes based on their due dates
    // For now, we'll just set a flag or navigate to a general review mode
    setQuizView("generalReview");
  };

  const startWeakQuestionsReview = () => {
    // This would review questions with low accuracy across all quizzes
