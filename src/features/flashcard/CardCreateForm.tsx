...
const CardCreateForm: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (cards.some((card) => !card.front.trim() || !card.back.trim())) {
      console.error("All cards must have front and back content");
      return;
    }

    window.deckIpc.deckSave(getPayload()).then(async (res) => {
      if (res.success) {
        try {
          // Get the created deck ID from the response
          const deckId = res.data?.deckId;
          if (deckId) {
            // Schedule all cards in the new deck for spaced repetition
            await window.deckIpc.scheduleDeckCards(deckId);
          } else {
            // Fallback: schedule all cards across all decks
          ...
          // Still proceed even if scheduling fails
          fetchDecks();
          setCardView("listing");
        }
      } else {
