...
function FileTree(
	...
	useEffect(() => {
		window.fileSystem.getNotesDir().then((root: string) => {
	...
	const handleDeleteMultiple = async () => {
		...
		for (const nodeId of selectedItems) {
			const result = await window.fileSystem.deleteFileOrFolder(nodeId);
			if (result.success) {
	...
	const handleRename = async (nodeId: string, newNodeName: string) => {
		...
		const result = await window.fileSystem.renameFileOrFolder(
			nodeId,
			newNodeName,
		);
		...
		async (args: {
			...
			for (const dragId of dragIds) {
				...
				if (currentParentPath !== newParentPath) {
					const result = await window.fileSystem.moveFile(
						dragId,
						newParentPath,
					);
					if (result.success) {
			...
			await window.fileSystem.updateFileOrders(ordersToUpdate);
			await loadNotes();
		},
